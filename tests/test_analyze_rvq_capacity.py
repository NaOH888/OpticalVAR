...
class AnalyzeRVQCapacityTests(unittest.TestCase):
    def test_analyze_rvq_capacity_runs_on_exported_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("pca_only_mad_to_original", summary)
            self.assertIn("rvq_full_mad_to_original", summary)
            self.assertIn("rvq_vs_pca_only_mad", summary)
            self.assertTrue((analysis_dir / "summary.json").exists())
            self.assertTrue((analysis_dir / "pca_vs_rvq_full.png").exists())
